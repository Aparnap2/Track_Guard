"""Test that Dockerfile.alpine exists for memory-service."""
import os
...
def test_memory_service_dockerfile_exists():
    """memory-service should have Dockerfile.alpine."""
    dockerfile = Path(__file__).parent / "memory-service" / "Dockerfile.alpine"
    assert dockerfile.exists(), f"Dockerfile not found: {dockerfile}"


def test_memory_service_dockerfile_is_valid():
